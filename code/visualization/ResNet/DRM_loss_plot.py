...
# %%
loss_matrix = np.load("loss_matrix_DRM.npy")
...
plt.title('Loss lansdcape of DRM (ResNet)')
ax.set_xlabel('Direction 1', size = 12) 
ax.set_ylabel('Direction 2', size = 12)
ax.set_zlabel('Loss', size = 12)

plt.savefig("Loss_landscape3D_of_DRM_ResNet.eps", dpi = 120)
plt.show()
...
plt.savefig("Loss_landscape2D_of_DRM_ResNet.eps", dpi = 120)
mp.show()
