...
loss_else_record = np.zeros(epoch)
...
for i in range(epoch):
    ...
    loss_else = loss_function_else(x)
    loss_else_record[i] = float(loss_else)
    error = error_function(x)
    error_record[i] = float(error)
    print("current epoch is: ", i)
    print("current DRM loss is: ", loss.detach())
    print("current DGM loss is: ", loss_else.detach())
    ...
    loss.backward()
    optimizer.step() 
    np.save("loss_of_DRM_200.npy", loss_record)
    np.save("loss_else_of_DRM_200.npy", loss_else_record)
    np.save("error_of_DRM_200.npy", error_record)
    np.save("eigen_vals_of_DRM_200.npy", eigen_vals_record)
np.save("loss_of_DRM_200.npy", loss_record)
np.save("loss_else_of_DRM_200.npy", loss_else_record)
np.save("error_of_DRM_200.npy", error_record)
np.save("eigen_vals_of_DRM_200.npy", eigen_vals_record)
